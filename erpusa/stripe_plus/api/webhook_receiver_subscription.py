...
def receive_stripe_subscription_events(data, type):
    if data.get("object") == "subscription":
        process_subscription_object(data)
    
    # make payment entry if subscription was succesfully set up
    if data.get("object") == "invoice":
        process_invoice_object(data, type)

def process_subscription_object(data):
    # get metadata to look for associated ERPNext subscription
    subscription_name = data.get("metadata", {}).get("erp_subscription_name")

    # check if subscription exists and set the stripe id and status
    if not subscription_name:
        return
    
    # check if subscription already has stripe_subscription_id (stripe subscription is new)
    if not frappe.db.get_value("Subscription", subscription_name, "stripe_subscription_id"):
        susbcription_id = data.get("id")
        frappe.db.set_value("Subscription", subscription_name, "stripe_subscription_id", susbcription_id)

        # if stripe is already linked with ERPNext, update the cancellation date if appplicable
        if not frappe.db.get_value("Subscription", subscription_name, "stripe_subscription_status") and frappe.db.get_value("Subscription", subscription_name, "end_date"):
            import stripe
            stripe.api_key = get_api_key_secret(payment_gateway=frappe.db.get_value("Subscription", subscription_name, "payment_gateway"))
            stripe.Subscription.modify(
                susbcription_id, 
                cancel_at=int(
                    (datetime.datetime.combine(frappe.db.get_value("Subscription", subscription_name, "end_date"), datetime.time()))
                    .timestamp()
                )
            )

        # if the stripe subscription is about to be active, send a welcome message and create a user before updating the subscription's stripe_subscription_status field
        if not frappe.db.get_value("Subscription", subscription_name, "stripe_subscription_status") and data.get("status") == "active":
            representative = frappe.db.get_value("Subscription", subscription_name, "user_account_representative")
            email_address = get_representative_email_address(
                representative=representative,
                log_title=f"Failed to send a welcome email for {subscription_name}.",
                as_dict=False
            )
            
            if not email_address:
                notify_error_to_user_new_subscription(subscription_name, susbcription_id, "email_address_missing")
            
            else:
                frappe.sendmail(
                    subject=_("Welcome to {}").format(frappe.db.get_value("Subscription", subscription_name, "company")),
                    recipients=[email_address],
                    message=frappe.render_template(
                        "erpusa/templates/html/subscription_welcome.html",
                        {
                            "customer": frappe.db.get_value("Subscription", subscription_name, "party"),
                    ...
                    reference_name=f"{subscription_name}_welcome"
                )
            
            user_to_authorize = frappe.db.get_single_value("Stripe Plus Settings", "user_to_authorize")

            if not user_to_authorize:
                notify_error_to_user_new_subscription(subscription_name, susbcription_id, "authorized_user_unset")
            else:
                ...
                if not user_exists:
                    ...
                    customer = frappe.get_doc("Customer", frappe.db.get_value("Subscription", subscription_name, "party"))
    ...
    frappe.db.set_value("Subscription", subscription_name, "stripe_subscription_status", SUBSCRIPTION_STATUS_VERBOSE[data.get("status")])

def notify_error_to_user_new_subscription(subscription_name, subscription_id, error_type):
    ...
    reference_name = f"{subscription_name}_{error_type}"
    if frappe.db.exists("Email Queue", {"reference_doctype": "Stripe Payout", "reference_name": reference_name}):
        return

    message = frappe.render_template(
        "erpusa/templates/html/new_subscription_errors.html", {
            "subscription_name": subscription_name,
            "subscription_link": get_url_to_form("Subscription", subscription_name),
    ...
    frappe.sendmail(
        ...
        subject=_("Stripe Subscription Failed"),
        message=message,
        reference_doctype="Subscription",
...
def process_invoice_object(data, type):
...
def create_payment_entry_from_stripe_invoice(invoice, subscription, reference_date=None, return_invoice_id=False):
    ...
    if charge:
        ...
        mp_doc = create_update_stripe_transaction(data=charge, api_key=api_key, return_mp_doc=True)
    else:
    ...
    frappe.log_error(str(payment_intent))
