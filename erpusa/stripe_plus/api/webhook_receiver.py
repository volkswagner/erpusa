...
def create_update_merchant_payment(stripe_transaction, metadata, api_key):
    ...
    if frappe.db.exists("Merchant Payment", mp_doc.name) and frappe.db.get_value("Stripe Transaction", mp_doc.source, "status") == "succeeded":
        frappe.enqueue(
            ...
            for index, source in enumerate(sources):
                if source.get('merchant_payment'):
                    ...
                    if not credit_account:
                        credit_account = frappe.db.get_single_value("Stripe Plus Settings", "credit_account")
