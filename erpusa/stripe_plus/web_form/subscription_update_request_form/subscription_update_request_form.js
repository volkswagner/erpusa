frappe.ready(function() {
	...
	const is_cancellation = {{ is_cancellation | tojson }};
	const is_resubscription = {{ is_resubscription | tojson }};
	console.log(is_new)
	...
	if (is_new) {
		...
		if (!(is_cancellation || is_resubscription)) {
			...
			frappe.web_form.on("change_end_date", function() {
				if (frappe.web_form.get_value("change_end_date") == 1) {
					frappe.web_form.set_df_property(
						"new_end_date",
						"reqd",
						1
					);
					frappe.web_form.set_value("new_end_date", "{{ end_date }}");
				}
				else {
					frappe.web_form.set_df_property(
						"new_end_date",
					...
					frappe.web_form.set_value("new_end_date", null);
				}
			});
		}
		else {
			frappe.web_form.set_df_property("request_type", "read_only", 1);
			frappe.web_form.set_df_property("change_end_date", "hidden", 1);
			frappe.web_form.set_df_property("plans", "hidden", 1);
			if (is_cancellation) {
				frappe.web_form.set_df_property("additional_information", "label", "Cancellation Reason");
